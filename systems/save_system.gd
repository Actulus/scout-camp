...
# Resolved once in _ready(): project folder in editor, next to the .exe in exports.
...
func _ready() -> void:
	if OS.has_feature("editor"):
		_save_path = ProjectSettings.globalize_path("res://scoutcamp_save.cfg")
	else:
		_save_path = OS.get_executable_path().get_base_dir().path_join("scoutcamp_save.cfg")
	set_process(false)
...
func save() -> void:
	...
	# Store each skill as its own key to avoid Dictionary serialization quirks
	for skill_id in GameManager.skills_completed:
		cfg.set_value("skills", skill_id, GameManager.skills_completed[skill_id])
...
func load_game_state() -> void:
	...
	GameManager.is_new_game = false  # suppress welcome popup on continue
	...
	# Restore each skill individually (avoids Dictionary serialization issues)
	for skill_id in GameManager.skills_completed:
		GameManager.skills_completed[skill_id] = cfg.get_value("skills", skill_id, false)
