...
func _input(event: InputEvent) -> void: 
	...
	if Input.is_action_pressed("lean_left"):
		target_lean = -1.0
	elif Input.is_action_pressed("lean_rigth"):
		target_lean = 1.0
	else:
		target_lean = 0.0 
	...
	if Input.is_action_just_pressed("inventory"):
		...
		# if the inventory is open show the cursor, inventory panel, and block all other interactiosn 
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
		inventory_controller.visible = true 
		interaction_raycast.enable = false 
...
func _process(delta: float) -> void:
	...
	if inventory_opened_flag and !Input.is_action_pressed("inventory"):
		...
		interaction_raycast.enabled = true 
		if not interaction_controller.current_object:
			# special check for interactable objects that still show the mouse (wheel) 
			Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
...
func updateCamera(delta: float) -> void: 
	...
	camera_3d.rotation.x = lerp(camera_3d.position.x, target_offset, delta*lean_speed)
